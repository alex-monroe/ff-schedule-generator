...
# team playing in a game in the given week
# Excludes variables representing the team playing itself
def get_teams_variables_for_week(
        variables, team, week, weeks_param, teams_param):
    teams_variables = []
    for w in weeks_param:
        for i in teams_param:
            for j in teams_param:
                if ((i == team) or (j == team)) and \
                   (i != j) and (w == week):
                    teams_variables.append(variables[i][j][w])
    return teams_variables


# Returns all of the variables in variables that represent team 1
# playing in team 2 in any week
def get_teams_variables_for_all_weeks(
...
def generate_schedule(num_weeks, num_teams):
    ...
    for j in range(num_teams):
        for k in range(num_teams):
            for w in range(num_weeks):
                variables[j][k][w] = solver.IntVar(
                    0, infinity, f"x[{j}][{k}][{w}]")
    ...
    for week in weeks:
        for team in teams:
            ...
            for variable in get_teams_variables_for_week(
    ...
    for team in teams:
        for team2 in teams:
            if (team != team2) and \
               ((team < num_teams/2 and team2 < num_teams/2) or
               (team >= num_teams/2 and team2 >= num_teams/2)):
                constraint = solver.RowConstraint(4, 4, "")
                for variable in get_teams_variables_for_all_weeks(
    ...
    for team in teams:
        for team2 in teams:
            if (team != team2) and \
               not ((team < num_teams/2 and team2 < num_teams/2) or
               (team >= num_teams/2 and team2 >= num_teams/2)):
                constraint = solver.RowConstraint(2, 2, "")
                for variable in get_teams_variables_for_all_weeks(
                        variables, team, team2, weeks, teams):
            ...
            for week in weeks[:-3]:
                if team > team2:
                    ...
                    constraint.SetCoefficient(
                        variables[team][team2][week], 1)
                    constraint.SetCoefficient(
                        variables[team][team2][week+1], 1)
                    constraint.SetCoefficient(
                        variables[team][team2][week+2], 1)
                    constraint.SetCoefficient(
                        variables[team][team2][week+3], 1)
