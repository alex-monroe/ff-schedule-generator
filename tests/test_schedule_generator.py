...
class TestScheduleGenerator(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.mock_variables = [
            [[f'var_{i}_{j}_{w}' for w in self.weeks]
                for j in self.teams] for i in self.teams
        ]

    def test_get_teams_variables_for_week(self):
        # Test for team 3, week 5
        result = schedule_generator.get_teams_variables_for_week(
        ...
        result_invalid_team = schedule_generator.get_teams_variables_for_week(
            self.mock_variables, 99, 5, self.weeks, self.teams)
        self.assertEqual(result_invalid_team, [])

        # Test with a week that doesn't exist
        result_invalid_week = schedule_generator.get_teams_variables_for_week(
            self.mock_variables, 3, 99, self.weeks, self.teams)
        self.assertEqual(result_invalid_week, [])

    def test_get_teams_variables_for_all_weeks(self):
        # Test for team 1 vs team 8
        result = schedule_generator.get_teams_variables_for_all_weeks(
        ...
        result_invalid_teams = \
            schedule_generator.get_teams_variables_for_all_weeks(
    ...
    def test_get_schedule_data_no_duplicate_matchups(self):
        ...
        schedule_data = schedule_generator._get_schedule_data(  # noqa: E501
            variables, weeks_param, teams_param)
    ...
    def test_generate_schedule_optimal_solution(self, MockSolver):
        ...
        mock_solver_instance.Solve.return_value = \
            schedule_generator.pywraplp.Solver.OPTIMAL
        mock_solver_instance.Objective.return_value.Value.return_value = 0

        # Mock variables with solution values
        mock_variables_solution = [
            [[Mock() for _ in range(2)] for _ in range(2)] for _ in range(2)
        ]
        ...
        with patch('src.schedule_generator._get_schedule_data') \
                as mock_get_schedule_data:
    ...
    def test_generate_schedule_no_optimal_solution(self, MockSolver):
        ...
        mock_solver_instance.Solve.return_value = \
            schedule_generator.pywraplp.Solver.INFEASIBLE
